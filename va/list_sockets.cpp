...
void display_record ( ProcNet *Record, pid_t pid, char *pname )
{
     ...
     if ( Record->protocol == PROTOCOL_TCP ) {
          printf ( "TCP " );
     } else if ( Record->protocol == PROTOCOL_UDP ) {
          printf ( "UDP " );
     } else {
          printf ( "RAW " );
     }
     ...
     snprintf ( buf, sizeof ( buf ), "%s:%u ", inet_ntoa ( Record->local_addr ), Record->local_port );
     printf ( "%s %*s", buf, 21 - strlen ( buf ), "" );
     snprintf ( buf, sizeof ( buf ), "%s:%u", inet_ntoa ( Record->remote_addr ), Record->remote_port );
     printf ( "%s %*s", buf, 21 - strlen ( buf ), "" );
...
void read_proc_net ( void )
{
     ...
     if ( ( NetData = ( ProcNet* ) calloc ( sizeof ( ProcNet ), size ) ) == NULL ) {
          fatal ( "Unable to allocate memory" );
     }
     ...
     while ( ( protocol = read_tcp_udp_raw ( buf, sizeof ( buf ) ) ) != 0 ) {
          if ( i == size ) {
               size *= 2;
               if ( ( NetData = ( ProcNet* ) realloc ( NetData, ( sizeof ( ProcNet ) * size ) ) ) == NULL ) {
                    fatal ( "Unable to allocate memory" );
               }
          }
     ...
     while ( true ) {
          ...
          while ( ( procent = readdir ( proc ) ) != NULL ) {
               ...
               while ( ( fdent = readdir ( fd ) ) != NULL ) {
                    ...
                    if ( ( ptr = ( ProcNet* ) bsearch ( &st.st_ino, NetData, total, sizeof ( ProcNet ), compare ) ) != NULL ) {
