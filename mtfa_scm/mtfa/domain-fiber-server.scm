(add-to-load-path "/new_devs/usr/local/guile-new/share/guile/site/3.0/")

(define-module (mtfa domain-fiber-server)
  #:use-module (fibers)
...
(define (make-default-socket addr)
  (let ((sock (socket PF_UNIX SOCK_STREAM 0)))
    (setsockopt sock SOL_SOCKET SO_REUSEADDR 1)
    (fcntl sock F_SETFD FD_CLOEXEC)
    (bind sock PF_UNIX addr)
...
(define (handle-request handler request body)
  (cond
   ((not request)
    ;; Bad request.
    (values (build-response #:version '(1 . 1) #:code 400
...
(define*-public (mtfa-fibers-run-domain-server handler #:key
			    (host "/tmp/va-domain.sock")
			    (socket (make-default-socket host)))
  "Run the va internal fibers web server.
...
@example
 (define (handler request body)
   (values '((content-type . (text/plain)))
           \"Hello, World!\"))
 (mtfa-fibers-run-domain-server handler)
@end example
...
;; (define (go)
;;   (run-server handler #:host "/tmp/pippo"))
