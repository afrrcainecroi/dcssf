...
class mtfa_unique_lock
{
private:
    shared_mutex * _shm;
public:
    mtfa_unique_lock(shared_mutex & shm)
    {
        //cout << __func__ << ". UniqueLock" << endl;
        _shm=&shm;
        shm.lock();
    };
    ~mtfa_unique_lock()
    {
        //cout << __func__ << ". UniqueUnlock" << endl;
        _shm->unlock();
    };
};

class mtfa_shared_lock
{
private:
    shared_mutex * _shm;
public:
    mtfa_shared_lock(shared_mutex & shm)
    {
        //cout << __func__ << ". SharedLock" << endl;
        _shm=&shm;
        shm.lock_shared();
    };
    ~mtfa_shared_lock()
    {
        //cout << __func__ << ". SharedUnlock" << endl;
        _shm->unlock_shared();
    };
};
...
// template <class Mutex>
// inline
// unique_lock<Mutex>::unique_lock(mtfa::upgrade_lock<mutex_type>&& ul)
...
// template <class Mutex>
// inline
// unique_lock<Mutex>::unique_lock(mtfa::shared_lock<mutex_type>&& sl,
//                                 try_to_lock_t)
//     : __m_(nullptr), __owns_(false)
// {
//     if (sl.owns_lock())
