...
/*************A NEW VERSION OF CONCURRENT_HASH_MAP ****************/
#include <utility>
#include <functional>
#include <memory>
#include <atomic>
...
namespace       std {
	...
	constexpr const std::size_t DEFAULT_SLOTS_PER_BUCKET = 4;
	constexpr const std::size_t DEFAULT_SIZE =
	    (1U << 16) * DEFAULT_SLOTS_PER_BUCKET;
	constexpr const std::size_t LOCK_ARRAY_GRANULARITY = 0;
	constexpr const double DEFAULT_MINIMUM_LOAD_FACTOR = 0.05;
	constexpr const std::size_t NO_MAXIMUM_HASHPOWER =
	...
	using           partial_t = uint8_t;
	    ...
	    void copy_allocator(Allocator & dst, const Allocator & src,
				std::true_type) {
	    dst = src;
	} template <    typename Allocator >
	    void copy_allocator(Allocator & dst, const Allocator & src,
	    ...
	    void swap_allocator(Allocator & dst, Allocator & src,
				std::true_type) {
	    std::swap(dst, src);
	}
	...
	template < class Key, class Value, class Allocator,
	    class PartialKey,
	    std::size_t SLOTS_PER_BUCKET > class bucket_container {
	  public:
	    using key_type = Key;
	    using           mapped_type = Value;
	    using           value_type = std::pair < const Key,
	                    Value >;
	  ...
	  public:
		...
		const           value_type & element(size_type index) const {
			...
			const value_type *>(&values[index]);
		} value_type &  element(size_type index) {
		    return *reinterpret_cast <
			value_type * >(&values[index]);
		}

		const           key_type & key(size_type index) const {
		    return element(index).first;
		} const         key_type && movable_key(size_type index) {
		    return std::move(element(index).first);
		}
	    ...
	    bucket_container(size_type hashpower,
		...
		buckets(transfer
			(other.hashpower(), other, std::false_type())) {
	    }
	  ...
	  :	allocator(std::move(other.allocator))
		...
		buckets =
		    transfer(other.hashpower(), other, std::false_type());
		return *this;
	    }
		...
		bucket_allocator = allocator;
		// Regardless of whether we actually swapped the
		// allocators or not, it will
		// always be okay to do the remainder of the swap. This is 
		// because if the
		// allocators were swapped, then the subsequent operations 
		// are okay. If the
		// allocators weren't swapped but compare equal, then
		// we're okay. If they
		// weren't swapped and compare unequal, then behavior is
		// undefined, so
		// we're okay.
		...
		void set_element(size_type index, size_type slot,
				 partial_t partial_key, K && k, Args
				 && ... args) {
		bucket & b = buckets[index];
		assert(!b.occupied(slot));
		b.partial(slot) = partial_key;
		traits::construct(allocator,
				  std::addressof(b.element(slot)),
		...
		traits::destroy(allocator,
				std::addressof(b.element(slot)));
	    }

	    void            move_element(size_type dst_index,
					 size_type dst_slot,
					 size_type src_index,
					 size_type src_slot) {
		...
		bucket         *new_buckets =
		    transfer(new_size, *this, std::true_type());
		destroy_buckets();
		buckets = new_buckets;
		hashpower(new_size);
	    }
	  ...
	  private:
	    void            move_assign(bucket_container & src,
					std::true_type) {
		...
		if (allocator == src.allocator) {
		    ...
		    buckets =
			transfer(src.hashpower(), src, std::true_type());
		}
	    }
	    ...
	    void            move_or_copy(size_type dst_index,
					 size_type dst_slot, bucket & src,
					 size_type src_slot,
					 std::true_type) {
		set_element(dst_index, dst_slot, src.partial(src_slot),
			    src.movable_key(src_slot),
	    ...
	    void            move_or_copy(size_type dst_index,
					 ...
					 std::false_type) {
		set_element(dst_index, dst_slot, src.partial(src_slot),
			    src.key(src_slot), src.mapped(src_slot));
	    }
		...
		for (size_t i = 0; i < src.size(); ++i) {
			...
			if (src.buckets[i].occupied(j)) {
			    dst.move_or_copy(i, j, src.buckets[i], j,
					     move);
			}
		    }
		}
	    ...
	    typename        traits::template rebind_alloc < bucket >
		bucket_allocator;
	    std::atomic < size_type > hashpower_holder;
	    bucket         *buckets;
	};

	using           LOCKING_ACTIVE = std::integral_constant < bool,
	    true >;
	using           LOCKING_INACTIVE = std::integral_constant < bool,
	    false >;
	...
	template < uint8_t OFFSET_BITS, uint8_t SEGMENT_BITS,
	    class Allocator > class spinlock_dynarray {
	  private:
	    using traits =
		typename std::allocator_traits <
		Allocator >::template rebind_traits < spinlock >;
	  ...
	  public:
	    ...
	    static_assert(SEGMENT_BITS + OFFSET_BITS <=
			  sizeof(size_type) * 8,
	  ...
	  spinlock_dynarray(spinlock_dynarray && other) noexcept:allocator(std::
		      move(other.
			   allocator))
		, segment_allocator(allocator)
	    ...
	    void            emulate(const spinlock_dynarray & other)
		noexcept {
		resize(other.size());
		...
		for (size_type i = allocated_segments; i <= last_segment;
		     ++i) {
	  ...
	  private:
		...
		spinlock       *segment =
		    traits::allocate(allocator, SEGMENT_SIZE);
	    ...
	    void            move_assign(spinlock_dynarray
					&& other, std::true_type) {
		allocator = std::move(other.allocator);
		segment_allocator = allocator;
		segments = other.segments;
	    ...
	    void            move_container(spinlock_dynarray & other,
					   std::false_type) {
	...
	// elements only define a sequence of alternate hashings for
	// different hash
	// values, we only need to keep track of the hash values being
	// moved, rather
	...
	// bfs_slot holds the information for a BFS path through the
	// table.
#pragma pack(push, 1)
	template < private_impl::size_type SLOTS_PER_BUCKET > struct bfs_slot {
	    using           size_type = private_impl::size_type;
	    // The bucket of the last item in the path.
	    size_type       bucket;
	    // a compressed representation of the slots for each of the
	    // buckets in
	    // the path. pathcode is sort of like a base-slot_per_bucket
	    // number, and
	    // we need to hold at most MAX_BFS_PATH_LEN slots. Thus we
	    // need the
	    // maximum pathcode to be at least
		...
		static_assert(MAX_BFS_PATH_LEN - 1 <=
			      ...
			      " MAX_BFS_PATH_LEN - 1");
	                    static_assert(-1 >= std::numeric_limits <
					  decltype(depth) >::min(),
					  "The depth type must be able to hold a value of -1");
	                    bfs_slot() {
	    } bfs_slot      (const size_type b, const size_type p,
...
#pragma pack(pop)
	...
	// bfs_queue is the queue used to store bfs_slots for BFS cuckoo
	// hashing.
#pragma pack(push, 1)
	template < private_impl::size_type SLOTS_PER_BUCKET >
	  ...
	  bfs_queue()noexcept:first(0), last(0) {
	    ...
	    // The maximum size of the BFS queue. Note that unless it's
	    // less than
	    // slot_per_bucket()^MAX_BFS_PATH_LEN, it won't really mean
	    // anything.
	    ...
	    size_type       first;
	    // One past the index of the last_ item of the queue in the
	    // array.
	    size_type       last;

	    // returns the index in the queue after ind, wrapping around
	    // if
	    // necessary.
...
#pragma pack(pop)
	...
	class Allocator = std::allocator < pair < const Key,
                    Value >> >class concurrent_unordered_map {
      public:
	// types:
	using key_type = Key;
	using           mapped_type = Value;
	using           value_type = pair < const Key,
	                Value >;
	using           hasher = Hasher;
	...
	using           buckets_t = private_impl::bucket_container < Key,
	    Value,
	    Allocator,
	    partial_t,
	...
	using           bucket = typename buckets_t::bucket;
      public:
	    ...
	    concurrent_unordered_map < Key, Value, Hasher, Equality,
		Allocator > &delegate;
	  ...
	  public:
	    ...
	    using           reference = value_type &;
	    using           const_reference = const value_type &;
	    using           size_type = concurrent_unordered_map < Key,
		Value,
		Hasher,
		Equality,
		Allocator >::size_type;
		...
		using difference_type =
		    unsynchronized_view::difference_type;
		using           value_type =
		    unsynchronized_view::value_type;
		using           pointer =
		    unsynchronized_view::const_pointer;
		using           reference =
		    unsynchronized_view::const_reference;
		using           iterator_category =
		    std::bidirectional_iterator_tag;

		                const_local_iterator() = default;
		bool            operator==(const const_local_iterator &
					   other) const {
		...
		const_local_iterator operator--(int) {
	      ...
	      protected:
	      const_local_iterator(bucket * data, size_type slot) noexcept:data(data)
		    , slot(slot) {
		...
		static const_local_iterator end(bucket * data) {
		    return const_local_iterator(data,
						private_impl::
		...
		using           reference = unsynchronized_view::reference;
		    ...
		    return const_local_iterator::data->
			element(const_local_iterator::slot);
		}
		...
		local_iterator  operator--(int) {
	      ...
	      private:
	      local_iterator(bucket * data, size_type slot) noexcept:const_local_iterator(data, slot)
		{
		}
	    ...
	    template < typename bucket_iterator > class base_iterator {
	      public:
		void            increment() {
		    ++bucket_position;
		    if (bucket_position ==
			bucket_iterator::
			end(&((*buckets)[bucket_index]))) {
			++bucket_position;
			while (bucket_index < buckets->size()) {
			    bucket_position =
				bucket_iterator::
				begin(&((*buckets)[bucket_index]));
			    if (bucket_position !=
				bucket_iterator::
				end(&((*buckets)[bucket_index]))) {
				break;
	    ...
	    class           const_iterator:protected base_iterator <
		...
		const_iterator  operator--(int) {
	      ...
	      private:
		const_iterator(buckets_t * buckets, size_type bucket_index,
			       size_type slot)
	      :    buckets(buckets)
		    , bucket_index(bucket_index)
		    ,
		    bucket_iterator(buckets
				    [bucket_index !=
				     buckets->size()? bucket_index : 0],
				    slot) {
		}
		...
		iterator        operator--(int) {
	      ...
	      private:
		iterator(buckets_t * buckets, size_type bucket_index,
			 size_type slot)
	      :    buckets(buckets)
		    , bucket_index(bucket_index)
		    ,
	    ...
	    unsynchronized_view(concurrent_unordered_map < Key, Value,
				Hasher, Equality, Allocator > &delegate)
	  :	delegate(delegate) {
	    }
		...
		return iterator(&delegate.buckets, delegate.buckets.size(),
				0);
	    }
	    const_iterator  end() const noexcept {
		return const_iterator(&delegate.buckets,
				      delegate.buckets.size(), 0);
	    ...
	    // modifiers: template < typename ... Args >
	                    pair < iterator,
		...
		auto            b =
		    delegate.snapshot_and_lock_two <
		    private_impl::LOCKING_INACTIVE > (hv);
		table_position  pos =
		    delegate.cuckoo_insert_loop(hv, b, x.first);
		if (pos.status == ok) {
		...
		return std::
		    make_pair(iterator
			      (delegate.buckets, pos.index, pos.slot),
			      pos.status == ok);
	    }
	    pair < iterator, bool > insert(const value_type && x) {
		hash_value      hv = delegate.hashed_key(x.first);
		...
		pair < iterator, bool > try_emplace(const key_type & k,
						    Args && ... args) {
		...
		auto            b =
		    ...
		    private_impl::LOCKING_INACTIVE > (hv);
		table_position  pos =
		    delegate.cuckoo_insert_loop(hv, b, key);
		if (pos.status == ok) {
		    delegate.add_to_bucket(pos.index, pos.slot, hv.partial,
					   key, std::forward(obj));
		} else {
	    ...
	    template < class M >
		pair < iterator, bool > insert_or_assign(key_type && key, M
							 && obj) {
		hash_value      hv = delegate.hashed_key(key);
		auto            b =
		    delegate.snapshot_and_lock_two <
		...
		delegate.del_from_bucket(position.bucket_index,
					 position.slot);
		return iterator(delegate.buckets, position.bucket_index,
				position.slot);
	    }
		...
		const table_position pos =
		    delegate.cuckoo_find(key, hv.partial, guard.first(),
					 guard.second());
		if (pos.status == ok) {
		...
		return iterator(delegate.buckets, last.bucket_index,
				last.slot);
	    }
		...
		void merge(concurrent_unordered_map < Key, Value, H2, P2,
			   Allocator > &source) {
	    ...
	    // map operations: iterator find(const key_type & key) {
		const hash_value hashvalue = delegate.hashed_key(key);
		const auto      guard =
		    delegate.snapshot_and_lock_two <
		    private_impl::LOCKING_INACTIVE > (hashvalue);
		const table_position pos =
		    delegate.cuckoo_find(key, hashvalue.partial,
		...
		const table_position pos =
		    delegate.cuckoo_find(key, hashvalue.partial,
					 guard.first(), guard.second());
		if              (pos.status == ok) {
		    return const_iterator(delegate.buckets, pos.index,
					  pos.slot);
		} else {
		    return cend();
		}
		...
		auto            result =
		    insert(std::forward < key_type > (key));
		return result.first->second;
	    }
		...
		if              (it == end()) {
		    throw           std::
			out_of_range("key not found in table");
		} else {
		    return it->second;
		}
	    }
		...
		return std::numeric_limits <
		    size_type >::max() / SLOTS_PER_BUCKET;
	    ...
	    // hash policy: float load_factor() const noexcept {
		return delegate.load_factor();
	    } void          rehash(size_type n) {
		delegate.cuckoo_expand_simple <
		    private_impl::LOCKING_INACTIVE, manual_resize > (n);
	    }
	};
	...
	explicit        concurrent_unordered_map(size_type n = 16,
      ...
      :    allocator(allocator)
	    ...
	    locks(n)
	    ,
	    minimum_load_factor_holder(private_impl::
				       DEFAULT_MINIMUM_LOAD_FACTOR)
	    ,
	    maximum_hashpower_holder(private_impl::NO_MAXIMUM_HASHPOWER) {
	}
	template < typename InputIterator >
	    concurrent_unordered_map(InputIterator first,
				     InputIterator last, size_type n =
				     private_impl::DEFAULT_SIZE,
				     const hasher & hf =
				     hasher(), const key_equal & eql =
				     key_equal(),
	...
	concurrent_unordered_map & operator=(initializer_list <
					     value_type > il) {
	...
	void            swap(concurrent_unordered_map & other) noexcept {
	    all_buckets_guard < private_impl::LOCKING_ACTIVE >
		unlocker1(nullptr);
	    all_buckets_guard < private_impl::LOCKING_ACTIVE >
		unlocker2(nullptr);
		...
		unlocker1 =
		    snapshot_and_lock_all < private_impl::LOCKING_ACTIVE >
		    ();
		unlocker2 =
		    other.snapshot_and_lock_all <
		    private_impl::LOCKING_ACTIVE > ();
	    } else {
		unlocker2 =
		    other.snapshot_and_lock_all <
		    private_impl::LOCKING_ACTIVE > ();
	    ...
	    size_type       tmp =
		minimum_load_factor_holder.load(std::memory_order_acquire);
	    minimum_load_factor_holder.store(other.
					     minimum_load_factor_holder.load
					     (std::memory_order_acquire),
					     std::memory_order_release);
	    other.minimum_load_factor_holder.store(tmp,
						   std::
						   memory_order_release);

	    tmp = maximum_hashpower_holder.load(std::memory_order_acquire);
	    maximum_hashpower_holder.store(other.
					   maximum_hashpower_holder.load
					   (std::memory_order_acquire),
					   std::memory_order_release);
	    other.maximum_hashpower_holder.store(tmp,
						 std::
						 memory_order_release);
	}
	...
	experimental::optional < mapped_type > find(const key_type & key) const {
	    ...
	    const auto      guard =
		snapshot_and_lock_two < private_impl::LOCKING_ACTIVE >
		(hashvalue);
	    const table_position pos = cuckoo_find(key, hashvalue.partial,
						   guard.first(),
						   guard.second());
	    if              (pos.status == ok) {
		experimental::optional < mapped_type >
		    (buckets[pos.index].mapped(pos.slot));
	    }
	    return {
	    };
	}
	...
	// concurrent-safe modifiers: template < typename ... Args >
	...
	bool            insert(value_type && x) {
	    return upsert(std::move(x.first), std::move(x.second));
	}
	...
	size_type       update(const key_type & key,
	...
	size_type       update(const key_type & key, mapped_type && val) {
		...
		(hv);
	    const table_position pos =
		cuckoo_find(key, hv.partial, guard.first(),
			    guard.second());
	    if (pos.status == ok) {
	...
	using           locks_t =
	    private_impl::spinlock_dynarray <
	    16 - private_impl::LOCK_ARRAY_GRANULARITY,
	    private_impl::LOCK_ARRAY_GRANULARITY,
	    Allocator >;
	using           hash_value = private_impl::hash_value;

	static constexpr auto SLOTS_PER_BUCKET =
	    private_impl::DEFAULT_SLOTS_PER_BUCKET;
	static constexpr auto MAX_BFS_PATH_LEN =
	    private_impl::MAX_BFS_PATH_LEN;

	using           bfs_slot =
	    private_impl::bfs_slot < SLOTS_PER_BUCKET >;
	using           bfs_queue =
	    private_impl::bfs_queue < SLOTS_PER_BUCKET >;
	...
	// The partial key must only depend on the hash value. It cannot
	// change with
	// the hashpower, because, in order for `cuckoo_fast_double` to
	// work
	// properly, the alt_index must only grow by one bit at the top
	// each time we
	// expand the table.
	static partial_t partial_key(const size_type hash) {
	    const uint64_t  hash_64bit = hash;
	    const uint32_t  hash_32bit =
		(static_cast < uint32_t > (hash_64bit) ^ static_cast <
		 uint32_t > (hash_64bit >> 32));
	    const uint16_t  hash_16bit =
		(static_cast < uint16_t > (hash_32bit) ^ static_cast <
		 uint16_t > (hash_32bit >> 16));
	    const uint16_t  hash_8bit =
		(static_cast < uint8_t > (hash_16bit) ^ static_cast <
		 uint8_t > (hash_16bit >> 8));
	    return hash_8bit;
	}
	...
	// Status codes for internal functions enum operation_status {
	    ok,
	    failure,
	...
	// A composite type for functions that need to return a table
	// position, and
	// a status code.
	...
	// hashed key
	// could be. It takes the first possible bucket as a parameter.
	// Note that
	// this function will return the first possible bucket if index is 
	// the
	// second possible bucket, so alt_index(ti, partial, alt_index(ti, 
	// partial,
	// index_hash(ti, hv))) == index_hash(ti, hv).
	static inline size_type alt_index(const size_type hashpower,
					  const partial_t partial,
					  const size_type index) {
	    // ensure tag is nonzero for the multiply. 0xc6a4a7935bd1e995
	    // is the
	    // hash constant from 64-bit MurmurHash2
	    const size_type nonzero_tag =
		static_cast < size_type > (partial) + 1;
	    return (index ^ (nonzero_tag * 0xc6a4a7935bd1e995)) &
		hashmask(hashpower);
	}

	size_type       hashpower() const {
	    return buckets.hashpower();
	}
	// try_read_from_bucket will search the bucket for the given key
	// and return// the index of the slot if found, or -1 if not found.
	    template < typename K >
	int             try_read_from_bucket(const bucket & b,
					     const partial_t partial,
					     const K & key) const {
	    // Silence a warning from MSVC about partial being unused if
	    // is_simple.
	    (void) partial;
	    for (size_type i = 0; i < SLOTS_PER_BUCKET; ++i) {
		if (!b.occupied(i)
		    || (!is_simple && partial != b.partial(i))) {
		    continue;
	...
	template < typename K >
	    table_position cuckoo_find(const K & key,
				       const partial_t partial,
				       const size_type first,
				       const size_type second) const {
	    int             slot =
		try_read_from_bucket(buckets[first], partial, key);
	    if              (slot != -1) {
		return table_position {
		first, static_cast < size_type > (slot), ok};
	    }
	    slot = try_read_from_bucket(buckets[second], partial, key);
	    if (slot != -1) {
		return table_position {
		second, static_cast < size_type > (slot), ok};
	    }
	    return table_position {
	    0, 0, failure_key_not_found};
	}

	// lock_index converts an index into buckets to an index into
	// locks.
	static inline size_type lock_index(const size_type bucket_index) {
	    return bucket_index & (locks_t::max_size() - 1);
	}

	template < typename LOCK_TYPE > class bucket_guard {
	  public:
	    bucket_guard() {
	    }
	    bucket_guard(locks_t * locks, size_type index)
	  :	locks(locks, unlocker {
		      index}
	    ) {
	    }
	...
	template < typename LOCK_TYPE > class two_buckets_guard {
	  public:
	    two_buckets_guard() {
	    }

	    two_buckets_guard(locks_t * locks, size_type first,
			      size_type second)
	  :	locks(locks, two_buckets_unlocker {
		      first, second}) {
	    }
	...
	template < typename LOCK_TYPE > class all_buckets_guard {
	  public:
	...
	// This exception is thrown whenever we try to lock a bucket, but
	// the
	// hashpower is not what was expected
	class           hashpower_changed {
	};

	// After taking a lock on the table for the given bucket, this
	// function will
	// check the hashpower to make sure it is the same as what it was
	// before the
	// lock was taken. If it isn't unlock the bucket and throw a
	// hashpower_changed exception.
	template < typename LOCK_TYPE >
	    inline void check_hashpower(const size_type old_hashpower,
					const size_type lock) const {
	...
	// locks the given bucket index.// // throws hashpower_changed if it changed after taking the lock.
	    ...
	    inline bucket_guard < LOCK_TYPE >
	    lock_one(const size_type hashpower,
	...
	// one.// // throws hashpower_changed if it changed after taking the lock.
	    template < typename LOCK_TYPE >
	    ...
	    two_buckets_guard < LOCK_TYPE >
	    lock_two(const size_type hashpower, const size_type first,
		     const size_type second) const {
	...
	// snapshot_and_lock_all takes all the locks, and returns a
	// deleter object
	// that releases the locks upon destruction. Note that after
	// taking all the
	// locks, it is okay to resize the buckets_ container, since no
	// other threads
	...
	template < typename LOCK_TYPE >
	    std::pair < two_buckets_guard < LOCK_TYPE >,
	    bucket_guard < LOCK_TYPE >> lock_three(const size_type hp,
						   const size_type i1,
						   const size_type i2,
						   const size_type i3)
	    const {
	    std::array < size_type, 3 > l { {
	    lock_index(i1), lock_index(i2), lock_index(i3)}};
	    // Lock in order.
	    if (l[2] < l[1])
		std::swap(l[2], l[1]);
	    if (l[2] < l[0])
	    ...
	    return std::make_pair(two_buckets_guard < LOCK_TYPE >
				  (&locks, i1, i2),
				  bucket_guard < LOCK_TYPE >
				  ((lock_index(i3) == lock_index(i1)
				    || lock_index(i3) == lock_index(i2))
				   ? nullptr : &locks, i3));
	}

	// snapshot_and_lock_two loads locks the buckets associated with
	// the given
	// hash value, making sure the hashpower doesn't change before the 
	// locks are
	// taken. Thus it ensures that the buckets and locks corresponding 
	// to the
	// hash value will stay correct as long as the locks are held. It
	// returns
	// the bucket indices associated with the hash value and the
	// current
	// hashpower.
	template < typename LOCK_TYPE >
	    two_buckets_guard < LOCK_TYPE >
	    snapshot_and_lock_two(const hash_value & hashvalue) const {
	    while (true) {
		// Store the current hashpower we're using to compute the
		// buckets
		const size_type old_hashpower = hashpower();
		const size_type first =
		    index_hash(old_hashpower, hashvalue.hash);
		const size_type second =
		    alt_index(old_hashpower, hashvalue.partial, first);
		                try {
		    return lock_two < LOCK_TYPE > (old_hashpower, first,
						   second);
		}
		catch           (hashpower_changed &) {
		    // The hashpower changed while taking the locks. Try
		    // again.
		    continue;
		}
	    }
	...
	template < typename K, typename LOCK_TYPE >
		...
		case failure_table_full:
		    // Expand the table and try again, re-grabbing the
		    // locks
		    cuckoo_fast_double < LOCK_TYPE,
			automatic_resize > (old_hashpower);
		    guard =
			snapshot_and_lock_two < LOCK_TYPE > (hashvalue);
		    break;
		case failure_under_expansion:
		    // The table was under expansion while we were
		    // cuckooing. Re-grab the
		    // locks and try again.
		    guard =
			snapshot_and_lock_two < LOCK_TYPE > (hashvalue);
		    break;
	...
	// to free up
	// a slot on either of the insert buckets, which are assumed to be 
	// locked
	// before the start. On success, the bucket and slot that was
	// freed up is
	// stored in insert_bucket and insert_slot. In order to perform
	// the search
	// and the swaps, it has to release the locks, which can lead to
	// certain
	// concurrency issues, the details of which are explained in the
	// function.
	// If run_cuckoo returns ok (success), then `b` will be active,
	// otherwise it
	// will not.
	template < typename LOCK_TYPE >
	    operation_status run_cuckoo(two_buckets_guard < LOCK_TYPE >
					&guard, size_type & insert_bucket,
					size_type & insert_slot) {
	    // We must unlock the buckets here, so that cuckoopath_search
	    // and
	    // cuckoopath_move can lock buckets as desired without
	    // deadlock.
	    // cuckoopath_move has to move something out of one of the
	    // original
	    // buckets as its last operation, and it will lock both
	    // buckets and
	    // leave them locked after finishing. This way, we know that
	    // if
	    // cuckoopath_move succeeds, then the buckets needed for
	    // insertion are
	    // still locked. If cuckoopath_move fails, the buckets are
	    // unlocked and
	    // we try again. This unlocking does present two problems. The 
	    // first is
	    // that another insert on the same key runs and, finding that
	    // the key
	    // isn't in the table, inserts the key into the table. Then we 
	    // insert
	    // the key into the table, causing a duplication. To check for 
	    // this, we
	    // search the buckets for the key we are trying to insert
	    // before doing
	    // so (this is done in cuckoo_insert, and requires that both
	    // buckets are
	    // locked). Another problem is that an expansion runs and
	    // changes the
	    // hashpower, meaning the buckets may not be valid anymore. In 
	    // this
	    // case, the cuckoopath functions will have thrown a
	    // hashpower_changed
		...
		while (!done) {
		    const int       depth =
			cuckoopath_search < LOCK_TYPE > (hp, path,
							 guard.first(),
							 guard.second());
		    if (depth < 0) {
			break;
			...
			assert(insert_bucket == guard.first()
			       || insert_bucket == guard.second());
			assert(LOCK_TYPE() ==
			       private_impl::LOCKING_INACTIVE()
			       || !locks[lock_index(guard.first())].
			       try_lock(LOCK_TYPE()));
			assert(LOCK_TYPE() ==
			       private_impl::LOCKING_INACTIVE()
			       || !locks[lock_index(guard.second())].
			       try_lock(LOCK_TYPE()));
			assert(!buckets[insert_bucket].
			       occupied(insert_slot));
			done = true;
			break;
		    }
		}
		...
		// The hashpower changed while we were trying to cuckoo,
		// which means
		// we want to retry. b.first() and b.second() should not
		// be locked
		// in this case.
	...
	// search. It
	// starts with the i1 and i2 buckets, and, until it finds a bucket 
	// with an
	// empty slot, adds each slot of the bucket in the bfs_slot. If
	// the queue runs
	...
	template < typename LOCK_TYPE >
		...
		size_type       starting_slot =
		    x.pathcode % SLOTS_PER_BUCKET;
		for (size_type i = 0; i < SLOTS_PER_BUCKET && !q.full();
		     ++i) {
		    size_type       slot =
			(starting_slot + i) % SLOTS_PER_BUCKET;
		    auto            ob =
			lock_one < LOCK_TYPE > (hp, x.bucket);
		    ...
		    // If x has less than the maximum number of path
		    // components,
		    // create a new bfs_slot item, that represents the
		    // bucket we would
		    // have come from if we kicked out the item at this
		    // slot.
	...
	// buckets to
	// an empty slot in another bucket. It returns the depth of the
	// discovered
	// cuckoo path on success, and -1 on failure. Since it doesn't
	// take locks on
	// the buckets it searches, the data can change between this
	// function and
	...
	template < typename LOCK_TYPE >
	    ...
	    // Fill in the cuckoo path slots from the end to the
	    // beginning.
	    ...
	    // to the
	    // end, using the final pathcode to figure out which bucket
	    // the path
	    // starts on. Since data could have been modified between
	    // slot_search
	    // and the computation of the cuckoo path, this could be an
	    // invalid
	    // cuckoo_path.
	    ...
	    {
		const auto      guard =
		    lock_one < LOCK_TYPE > (hp, first.bucket);
		...
		const auto      guard =
		    lock_one < LOCK_TYPE > (hp, cur.bucket);
	...
	// Checks whether the resize is okay to proceed. Returns a status
	// code, or
	// throws an exception, depending on the error type.
	using           automatic_resize = std::integral_constant < bool,
	    true >;
	using           manual_resize = std::integral_constant < bool,
	    false >;
	...
	{
	    const size_type mhp =
		maximum_hashpower_holder.load(std::memory_order_acquire);
	    if (mhp != private_impl::NO_MAXIMUM_HASHPOWER && new_hp > mhp) {
		throw           private_impl::
		    maximum_hashpower_exceeded(new_hp);
	    }
	    if (AUTO_RESIZE::value
		&& load_factor() < minimum_load_factor()) {
		throw           private_impl::
		    load_factor_too_low(minimum_load_factor());
	    }
	...
	// cuckoo_fast_double will double the size of the table by taking
	// advantage
	// of the properties of index_hash and alt_index. If the key's
	// move
	// constructor is not noexcept, we use cuckoo_expand_simple, since 
	// that
	...
	template < typename LOCK_TYPE, typename AUTO_RESIZE >
		...
		mapped_type >::value) {
		return cuckoo_expand_simple < LOCK_TYPE,
		    AUTO_RESIZE > (current_hp + 1);
	    }
	    const size_type new_hp = current_hp + 1;
	    std::lock_guard < expansion_lock_t > l(expansion_lock);
	    auto            st =
		check_resize_validity < AUTO_RESIZE > (current_hp, new_hp);
	    if (st != ok) {
		return st;
	    }
	    ...
	    auto            unlocker =
		snapshot_and_lock_all < LOCK_TYPE > ();
	    buckets.resize(new_hp);

	    // We gradually unlock the new table, by processing each of
	    // the buckets
	    // corresponding to each lock we took. For each slot in an old 
	    // bucket,
	    // we either leave it in the old bucket, or move it to the
	    // corresponding
	    // new bucket. After we're done with the bucket, we release
	    // the lock on
	    // it and the new bucket, letting other threads using the new
	    // map
	    // gradually. We only unlock the locks being used by the old
	    // table,
	    // because unlocking new locks would enable operations on the
	    // table
	    // before we want them. We also re-evaluate the partial key
	    // stored at
	    ...
	    parallel_exec(0, locks_to_move,
			  [this, current_hp, new_hp] (size_type start,
						      size_type end,
						      std::
						      exception_ptr & eptr)
			  {
			  try {
			  move_buckets < LOCK_TYPE > (current_hp, new_hp,
						      start,
						      end);} catch( ...) {
			  eptr = std::current_exception();}
			  });
	    parallel_exec(locks_to_move, locks.size(),
			  [this] (size_type i, size_type end,
				  std::exception_ptr &) {
			  for (; i < end; ++i) {
			  locks[i].unlock(LOCK_TYPE());}
			  }
	    );
	    // Since we've unlocked the buckets ourselves, we don't need
	    // the
	...
	template < typename LOCK_TYPE >
	    void move_buckets(size_type current_hp, size_type new_hp,
			      size_type start_lock_ind,
			      size_type end_lock_ind) {
		...
		for (size_type bucket_i = start_lock_ind;
		    ...
		    // By doubling the table size, the index_hash and
		    // alt_index of
		    // each key got one bit added to the top, at position
		    // current_hp, which means anything we have to move
		    // will either
		    ...
		    const size_type new_bucket_i =
			bucket_i + hashsize(current_hp);
		    size_type       new_bucket_slot = 0;

		    // Move each item from the old bucket that needs
		    // moving into the
		    // new bucket
		    for (size_type slot = 0; slot < SLOTS_PER_BUCKET;
			 ++slot) {
			if (!old_bucket.occupied(slot)) {
			    continue;
			}
			const hash_value hv =
			    hashed_key(old_bucket.key(slot));
			const size_type old_ihash =
			    index_hash(current_hp, hv.hash);
			const size_type old_ahash =
			    alt_index(current_hp, hv.partial, old_ihash);
			const size_type new_ihash =
			    index_hash(new_hp, hv.hash);
			const size_type new_ahash =
			    alt_index(new_hp, hv.partial, new_ihash);
			if ((bucket_i == old_ihash
			     && new_ihash == new_bucket_i)
			    || (bucket_i == old_ahash
				&& new_ahash == new_bucket_i)) {
			    // We're moving the key from the old bucket to 
			    // the new
			    // one
			    move_element(new_bucket_i, new_bucket_slot++,
					 bucket_i, slot);
			} else {
			    // Check that we don't want to move the new
			    // key
			    assert((bucket_i == old_ihash
				    && new_ihash == old_ihash)
				   || (bucket_i == old_ahash
				       && new_ahash == old_ahash));
			}
		...
		// Now we can unlock the lock, because all the buckets
		// corresponding
	...
	static size_type reserve_calc(const size_type n) {
		...
		(n + SLOTS_PER_BUCKET - 1) / SLOTS_PER_BUCKET;
	    size_type       blog2;
	    for (blog2 = 0; (1UL << blog2) < buckets; ++blog2);
	    assert(n <= buckets * SLOTS_PER_BUCKET
		   && buckets <= hashsize(blog2));
	    return blog2;
	}

	// move_element moves a key-value pair from one location to
	// another, also updating
	// the lock counters. Assumes locks are already taken.
	void            move_element(size_type dst_bucket,
				     size_type dst_slot,
				     size_type src_bucket,
				     size_type src_slot) {
	    buckets.move_element(dst_bucket, dst_slot, src_bucket,
				 src_slot);
	...
	// cuckoo_expand_simple will resize the table to at least the
	// given
	// new_hashpower. When we're shrinking the table, if the current
	// table
	// contains more elements than can be held by new_hashpower, the
	// resulting
	// hashpower will be greater than new_hashpower. It needs to take
	// all the
	// bucket locks, since no other operations can change the table
	// during
	// expansion. Throws libcuckoo_maximum_hashpower_exceeded if we're 
	// expanding
	...
	template < typename LOCK_TYPE, typename AUTO_RESIZE >
	    ...
	    auto            st =
		check_resize_validity < AUTO_RESIZE > (hp, new_hp);
	    if (st != ok) {
		return st;
	    }
	    // Creates a new hash table with hashpower new_hp and adds all
	    // the elements from the old buckets.
	    concurrent_unordered_map new_map(hashsize(new_hp) *
					     SLOTS_PER_BUCKET,
					     hash_function(), key_eq(),
					     get_allocator());

	    parallel_exec(0, hashsize(hp),
			  [this, &new_map] (size_type i, size_type end,
	    ...
	    // Swap the current buckets containers with new_map's. This is 
	    // okay,
	    // because we have all the locks, so nobody else should be
	    // reading from the
	    // buckets array. Then the old buckets array will be deleted
	    // when new_map
	    // is deleted. We also need to `emulate` new_map's locks
	    // array, so we have
	    // the same size and data content. We can't swap the memory
	    // though, since
	    // other threads may still be looking at our lock memory.
	    // Regardless, we
	    // shouldn't need to swap the memory, since new_map is using
	    // the same
	    // allocator as we are.
	...
	// to make
	// an empty slot in one of the buckets in cuckoo_insert. Before
	// the start of
	// this function, the two insert-locked buckets were unlocked in
	// run_cuckoo.
	// At the end of the function, if the function returns true
	// (success), then
	...
	template < typename LOCK_TYPE >
		...
		// try_add_to_bucket sees
		// both buckets as full and cuckoopath_search finds one
		// empty. In
		// this case, we lock both buckets. If the slot that
		// cuckoopath_search found empty isn't empty anymore, we
		// unlock them
		// and return false. Otherwise, the bucket is empty and
		// insertable,
		// so we hold the locks and return true.
		const size_type bucket = path[0].bucket;
		assert(bucket == guard.first()
		       || bucket == guard.second());
		guard =
		    lock_two < LOCK_TYPE > (hp, guard.first(),
					    guard.second());
		...
		if (depth == 1) {
		    // Even though we are only swapping out of one of the
		    // original
		    // buckets, we have to lock both of them along with
		    // the slot we
		    // are swapping to, since at the end of this function, 
		    // they both
		    // must be locked. We store tb inside the extrab
		    // container so it
		    // is unlocked at the end of the loop.
		    std::tie(twob, extrab) =
			lock_three < LOCK_TYPE > (hp, guard.first(),
						  guard.second(),
						  to.bucket);
		} else {
		    twob =
			lock_two < LOCK_TYPE > (hp, from.bucket,
						to.bucket);
		}
		...
		// cuckoo. If
		// that happened, just... try again. Also the slot we are
		// filling in
		// may have already been filled in by another thread, or
		// the slot we
		// are moving from may be empty, both of which invalidate
		// the swap.
		// We only need to check that the hash value is the same,
		// because,
		// even if the keys are different and have the same hash
		// value, then
		// the cuckoopath is still valid.
		if (hashed_key_only_hash(from_bucket.key(from_slot)) !=
		    from.hv.hash || to_bucket.occupied(to_slot)
	...
	// slot. The key
	// and value will be move-constructed into the table, so they are
	// not valid
	// for use afterwards.
	template < typename K, typename ... Args >
	    void add_to_bucket(const size_type bucket_index,
			       const size_type slot,
			       const partial_t partial, K && key, Args
			       && ... val) {
	...
	template < typename K, typename ... Args >
		...
		buckets[pos.index].mapped(pos.slot) =
		    mapped_type(std::forward < Args > (val) ...);
	    }
	    return pos.status == ok;
	}
	...
	template < typename K, typename LOCK_TYPE >
		...
		return table_position {
		guard.first(), static_cast < size_type > (res1),
			failure_key_duplicated};
	    }
	    bucket & b2 = buckets[guard.second()];
	    if (!try_find_insert_bucket(b2, res2, hashvalue.partial, key)) {
		return table_position {
		guard.second(), static_cast < size_type > (res2),
			failure_key_duplicated};
	    }
	    if (res1 != -1) {
		return table_position {
		guard.first(), static_cast < size_type > (res1), ok};
	    }
	    if (res2 != -1) {
		return table_position {
		guard.second(), static_cast < size_type > (res2), ok};
	    }
	    // We are unlucky, so let's perform cuckoo hashing.
	    size_type       insert_bucket = 0;
	    size_type       insert_slot = 0;
	    auto            st =
		run_cuckoo < LOCK_TYPE > (guard, insert_bucket,
					  insert_slot);
	    if (st == failure_under_expansion) {
		// The run_cuckoo operation operated on an old version of
		// the table,
		// so we have to try again. We signal to the calling
		// insert method
		// to try again by returning failure_under_expansion.
		return table_position {
		0, 0, failure_under_expansion};
	    } else if (st == ok) {
		...
		assert(insert_bucket ==
		       index_hash(hashpower(), hashvalue.hash)
		       || insert_bucket == alt_index(hashpower(),
						     hashvalue.partial,
						     index_hash(hashpower
								(),
								hashvalue.
								hash)));
		// Since we unlocked the buckets during run_cuckoo,
		// another insert
		// could have inserted the same key into either b.first()
		// or
		// b.second(), so we check for that before doing the
		// insert.
		...
		return table_position {
		insert_bucket, insert_slot, ok};
	    }
	    assert(st == failure);
	    return table_position {
	    0, 0, failure_table_full};
	}
	...
	template < typename F >
		...
		threads[i] =
		    std::thread(func, start, start + work_per_thread,
				std::ref(eptrs[i]));
		start += work_per_thread;
	    }
	    threads.back() =
		std::thread(func, start, end, std::ref(eptrs.back()));
	...
	void            del_from_bucket(const size_type bucket_index,
					const size_type slot) {
	...
	void            minimum_load_factor(const double mlf) {
	    ...
	    minimum_load_factor_holder.store(mlf,
					     std::memory_order_release);
	}
	double          minimum_load_factor() {
	    return minimum_load_factor_holder.
	...
	void            maximum_hashpower(size_type mhp) {
	    if (hashpower() > mhp) {
		throw           std::
		    invalid_argument("maximum hashpower " +
				     std::to_string(mhp) +
	...
	size_type       maximum_hashpower() {
	    return maximum_hashpower_holder.
		load(std::memory_order_acquire);
	}

	double          load_factor() const {
	    return static_cast < double >(size()) / static_cast <
		double >(capacity());
	} size_type     size() const {
	    size_type       s = 0;
	    for             (size_type i = 0; i < locks.size(); ++i) {
